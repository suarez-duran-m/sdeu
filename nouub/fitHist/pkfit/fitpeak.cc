...
double fitpeak::getRms(vector < vector < double > > values, vector < double > average, int st)
{
  double rms = 0.;
...
double fitpeak::getRms(vector < vector < double > > values, vector < int > average, int st)
{
  double rms = 0.;
  for( unsigned int val = 0; val<values[st].size(); val++ )
    rms += (average[st] - values[st][val])*(average[st] - values[st][val]);
