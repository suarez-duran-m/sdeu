...
basenameUub = "uubAoPtimePMT"+pmtid+"St"+stid+"Mth" #+monthUub+".json"
basenameUb = "../../nouub/fitHist/aoptime/ubAoPtimePMT"+pmtid+"St"+stid+"Mth" #+monthUb+".json"
outnameHbasepk = "../../plots/uububAoPtimeHbPMT"+pmtid+"St"+stid+"pk.png"
outnameHbasech = "../../plots/uububAoPtimeHbPMT"+pmtid+"St"+stid+"ch.png"
...
for month in monthUub:
    ...
    for info in tmpdata[month]:
        ...
        yaopHb[1].append( info['pk']*factor )
        yaopHberr[1].append( info['pkrms'] )
...
plt.xlabel("Days since December 1st, 2020 (month/day)", fontsize=24)
plt.ylabel("Average VEM-Peak per day [FADCp]", fontsize=24)
plt.xticks(fontsize=22)
plt.yticks(fontsize=22)
#plt.ylim(1, 4)
plt.tight_layout()
plt.show()
#plt.savefig(outnameHbasepk, dpi=100)
...
for month in monthUub:
    ...
    for info in tmpdata[month]:
        if info['Date'] < 1606867200:
            continue
        xtime[1].append( datetime.datetime.fromtimestamp( info['Date'] ) )
        yaopHb[1].append( info['ch']*factor )
        yaopHberr[1].append( info['chrms'] )
...
plt.legend(title="St. "+stid+" PMT"+pmtid, fontsize=22, title_fontsize=22)
plt.xlabel("Days since December 1st, 2020 (month/day)", fontsize=24)
plt.ylabel("Average VEM-Charge per day [FADCq]", fontsize=24)
plt.xticks(fontsize=22)
plt.yticks(fontsize=22)
plt.ylim(1e3, )
plt.tight_layout()
plt.show()
#plt.savefig(outnameHbasech, dpi=100)
print("Average AoP Peak-HBase OK")
