...
int main (int argc, char *argv[]) {
  ...
  //TFile hfile("ubCalibHist"+pmtname+"Aug.root","RECREATE","");
  ...
  TH1F *offSetPk = new TH1F ("offSetPk","", totSt, 0, totSt);// Receive Peak offset from IoSdHisto::Histo for St.
    ...
    for (unsigned int i = 0 ; i < event.Stations.size(); ++i){
      ...
      for ( unsigned int stid = 0; stid<totSt; stid++ )
        ...
        {          
          ...
          fitHist.getFitPk(*tmp, 0.2, 5, event.Stations[i].Calib->VemPeak[pmtId-1]);
          ...
          fitHist.getChCrrOst(*receCh, blCorrCalib*0., tmpName+"Clch");
          tmp = fitHist.getChCorr();
