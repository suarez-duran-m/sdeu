...
void readHistos::getPkCrrOst(TH1F &hist, const int bl, TString name) {

	unsigned int nb = 150;
	double xb [nb];

	for ( unsigned int b=0; b<nb; b++ )
		xb[b] = hist.GetBinLowEdge(b+1) - bl;
...
void readHistos::getChCrrOst(TH1F &hist, const int bl, TString name) {

	unsigned int nb = 600;
	double xb [nb];

	for ( unsigned int b=0; b<nb; b++ )
		xb[b] = hist.GetBinLowEdge(b+1) - 20.0*bl;
	xb[nb] = xb[nb-1] + hist.GetBinWidth(1);
...
void readHistos::getFitCh(TH1F &hist, const double frac, const int fstbinFit, const double vemch ) {
	...
	//fitFcn->SetParameters(11.61, (rangXmax+rangXmin)/2., 0.3, 7.8, 45.5);// For No baseline
