...
void readFitPeakHisto()
{
  ...
  peak->GetXaxis()->SetRangeUser(0, 240);
  peak->SetLineWidth(1);
  ...
  peakDer->GetXaxis()->SetRangeUser(0, 240);
  peakDer->GetXaxis()->SetTitle("[FADC]");
  peakDer->GetYaxis()->SetRangeUser(-300, 1200);
  peakDer->GetYaxis()->SetTitle("[au]");
  histoStyle(peakDer);
  peakDer->Draw();

  line = new TLine(0, 0, 240, 0);
  line->SetLineStyle(4);
  line->SetLineWidth(1);
  line->Draw();
  ...
  leg->AddEntry(peakFFTDer,"First derivative FFT","f");
  leg->Draw();
  c3->Print("../../plots/ubpeakSmoothDerHisto863.pdf");
  ...
  for ( int kk=75; kk>27; kk-- ) // from 300 FADC backward
    if ( peakSmooDer->GetBinContent(kk) < 0 )
    {
      ...
      {
        binMax = fabs(peakSmooDer->GetBinContent(kk));
        rangXmax = peakSmooDer->GetBinCenter(kk);
      }
    }
    else
    {
      binMax = peakSmooDer->GetBinCenter(kk);
      break;
    }
  ...
  for ( int kk=7; kk<binMax; kk++ ) // 20 FADC after 0 FADC
  {
    if ( peakSmooDer->GetBinContent( kk ) > 0 && tmpneg == 1 )
      break;
    if ( peakSmooDer->GetBinContent( kk ) < 0 )
      if ( binMin < fabs( peakSmooDer->GetBinContent( kk ) ) )
      {
        rangXmin = peakSmooDer->GetBinCenter(kk);
  ...
  chFit->SetTitle("");
  chFit->GetXaxis()->SetRangeUser(0, 240);
  ...
  c4->Print("../../plots/ubpeakFittedHisto863.pdf");
  ...
  residGraph->GetYaxis()->SetRangeUser(-4, 6);
  ...
  line = new TLine(peakVal, -4, peakVal, 6);
  line->SetLineStyle(4);
  line->SetLineWidth(2);
  line->Draw();
  ...
  c6->Print("../../plots/ubpeakFitResiduals863.pdf");
  ...
  poly2Fit->SetTitle("");
  poly2Fit->GetXaxis()->SetRangeUser(0, 250);
  ...
  c8->Print("../../plots/ubpeakFitResidualsPoly2863.pdf");
  ...
  c9->Print("../../plots/ubpeakSmoothFitPoly2863.pdf");
  ...
  c11->Print("../../plots/ubpeakSmoothFittedHisto863.pdf");
