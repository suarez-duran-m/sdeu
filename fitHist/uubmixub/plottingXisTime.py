...
outnameHbase = "../../plots/uububChistimeHbSt"+str(stList[stchoo])+"PMTs.png"
  ...
  for month in monthUb:
    ...
    for st in range(len(stList)):
      for info in tmpdata[str(stList[st])]:
        ...
        chirmshb[0][st].append( info['ChirmsHb'] )
  ...
  for month in monthUub:
    ...
    for st in range(len(stList)):
      for info in tmpdata[str(stList[st])]:
        ...
        chirmshb[1][st].append( info['ChirmsHb'] )

  return xtime, chihb, chirmshb
...
ax1.legend(loc=2, title='PMT1', fontsize=12)
ax1.xaxis.set_major_formatter(xfmt)
ax1.set_ylim(ymin, ymax)
ax1.set_ylabel("$\chi^{2}/$NDF [FADC]", fontsize=12)
...
ax2.legend(loc=2, title='PMT2', fontsize=12)
ax2.xaxis.set_major_formatter(xfmt)
ax2.set_ylim(ymin, ymax)
ax2.set_ylabel("$\chi^{2}/$NDF [FADC]", fontsize=12)
...
ax3.legend(loc=2, title='PMT3', fontsize=12)
ax3.xaxis.set_major_formatter(xfmt)
ax3.set_xlabel("Days since August 1st, 2020 (month/day)", fontsize=12)
ax3.set_ylim(ymin, ymax)
ax3.set_ylabel("$\chi^{2}/$NDF [FADC]", fontsize=12)
