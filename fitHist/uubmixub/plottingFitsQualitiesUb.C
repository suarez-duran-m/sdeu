...
void plottingFitsQualitiesUb(int st)
{
  TString stat;
  TString basename = "../../nouub/fitHist/aoptimeTraceSelec/ubAoPtimePMT";
  stat.Form("St%d", st);
  TString pmtId = "1";
  ...
  for ( int month=0; month<4; month++ )
  {
    ...
    {
      tmp.Form("%d", pmt);
      f = TFile::Open(basename+tmp+stat+"Mth"+monthUb[month]+".root");
      ...
      for( int etry=0; etry<histos->GetEntries(); etry++)
      {
        ...
        if ( tmpChiPk > 10. ) //|| tmpPeak < 30. )
          tmpChiPk = 10.;
        if ( tmpChiCh > 10. ) //|| tmpCharge < 50. )
          tmpChiCh = 10.;
  ...
  int tmpMonth = 0; //rdmMonth->Integer(4);
  ...
  {
    tmp.Form("%d", pmt);
    tmpMonth = 0; // rdmMonth->Integer(4);
    f = TFile::Open(basename+tmp+stat+"Mth"+monthUb[tmpMonth]+".root");
    ...
    nbins = (chiGrpPk->GetPointX(nPoints-1) - chiGrpPk->GetPointX(0) ); // Binwidth for Peak histograms
    ...
    for ( int ntry=0; ntry<histos2->GetEntries(); ntry++ )
    {
      ...
      if ( tmpChiPk > tmpAvepk-0.5*tmpRmspk && tmpChiPk < tmpAvepk+0.5*tmpRmspk ) //&& tmpPeak > 30. )
    ...
    residMean->GetXaxis()->SetTitleSize(0.05);
    residMean->GetXaxis()->SetTitle("Peak [FADC]");
    residMean->GetXaxis()->SetRangeUser(0, 120);
    residMean->GetYaxis()->SetTitle("Counts [au]");
    residMean->GetYaxis()->SetTitleSize(0.05);
    residMean->GetYaxis()->SetTitleOffset(1.);
    ...
    rpmean->GetLowerRefYaxis()->SetLabelSize(0.04);
    if ( pmt==1 )
      pad1->Update();
    else if ( pmt==2 )
      pad2->Update();
    ...
    residMean1->GetXaxis()->SetRangeUser(0, 120);
  ...
  tmpMonth = 1; //rdmMonth->Integer(5);

  for ( int pmt=1; pmt<4; pmt++ )
  {
    tmp.Form("%d", pmt);
    while ( tmpMonth==0 )
      tmpMonth = rdmMonth->Integer(4);
    f = TFile::Open(basename+tmp+stat+"Mth"+monthUb[tmpMonth]+".root");
    ...
    nbins2 = (chiGrpCh->GetPointX(nPoints2-1) - chiGrpCh->GetPointX(0));
    ...
    for ( int ntry=0; ntry<histos3->GetEntries(); ntry++ )
    {
      ...
      if ( tmpChiCh > tmpAvech-0.5*tmpRmsch && tmpChiCh < tmpAvech+0.5*tmpRmsch ) //&& tmpCharge > 50. )
        tmpInMeanch.push_back( ntry );
      if ( tmpChiCh > tmpAvech + 1.5*tmpRmsch )
    ...
    residMean2->GetXaxis()->SetRangeUser(0, 300);
    ...
    residMean21->GetXaxis()->SetRangeUser(0, 300);
