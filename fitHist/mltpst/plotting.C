void plotting(int pmt) {
	...
	TFile *f = TFile::Open("uubCalibHist"+pmtId+".root");
	cout << "Reading file: " << "uubCalibHist"+pmtId+".root" <<endl;
	...
	*/
	...
	int totSt = 19;
	double blHbase[totSt]; 
	double blCalib[totSt];
	int offsetpk[totSt];
	int offsetch[totSt];
	//double chpk[totSt];
	int evtTime;
	//int firstCntBinPk[totSt];
	int firstCntBinCh[totSt];
